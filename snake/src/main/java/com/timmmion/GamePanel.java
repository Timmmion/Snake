...
public class GamePanel extends JPanel implements ActionListener{
        ...
        if(running){
            ...
            for(int i = 0; i < bodyParts; i++){
                if(i == 0){
                    ...
                    g.setColor(Color.decode("#197419"));
                    g.fillRect(x[i], y[i], UNIT_SIZE, UNIT_SIZE);
                }
            }

            g.setColor(Color.red);
            g.setFont(new Font("SEOGE UI", Font.BOLD, 35));
    ...
    public void gameOver(Graphics g){
        ...
        //GAME OVER TEXT
        g.setColor(Color.red);
        g.setFont(new Font("SEOGE UI", Font.BOLD, 75));
        g.drawString("Game Over!", (SCREEN_WIDTH - getFontMetrics(g.getFont()).stringWidth("Game Over")) / 2, SCREEN_HEIGHT/ 2);

        //SCORE TEXT
        g.setColor(Color.red);
        g.setFont(new Font("SEOGE UI", Font.BOLD, 35));
        g.drawString("Score: " + applesEaten, (SCREEN_WIDTH - getFontMetrics(g.getFont()).stringWidth("Score: " + applesEaten)) / 2, SCREEN_HEIGHT / 2 + 50);
    }
